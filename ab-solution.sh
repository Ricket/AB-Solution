...
if [ -f $jScripts/post-mount ];then
	...
	if grep -q "# generated by AB-Solution 1.07" $jScripts/post-mount;then
		...
		# unless sleep time is 21 or 22 (fix)
		if grep -q "sleep 21\|sleep 22" $jScripts/post-mount;then
		# fixing sleep time
		echo " Fixing the incorrect sleep time in post-mount"
		sed -i -e 's/sleep 21/sleep 2/g;s/sleep 22/sleep 2/g' $jScripts/post-mount
		fi
	else
	# make a backup of the file
