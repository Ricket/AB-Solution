...
sed -i '/^[[:blank:]]*$/d' $jScripts/services-start
	if grep -q "# generated by\|UpdateHosts\|restart_dnsmasq;logger" $jScripts/services-start;then
		# remove previous entries and empty lines. no longer needed
...
if [ -s $jScripts/services-start ]; then
	if ! test $(sed -n 2p < $jScripts/services-start); then
	rm -f $jScripts/services-start
	fi
else
	rm -f $jScripts/services-start
fi
...
if [ -f $jScripts/post-mount ];then
	...
	sed -i '/^[[:blank:]]*$/d' $jScripts/post-mount
	if ! grep -q "AB-Solution 1.05" $jScripts/post-mount ;then
		if grep -q "#!/bin/" $jScripts/post-mount ;then
...
if [ $prevSettings == "1" ] && [ $updating == "1" ];then
	...
	echo -e "    Filesize: ~6 MB, ~201'000 blocked hosts.\n"
...
if [ $loggingState == "on" ];then
	...
	echo -e " tail -f $installDirPath/$logsDir/dnsmasq.log\n"
...
# find out what to do 
if [ -f $jScripts/dnsmasq.postconf ];then
	# if file found, but not written by this script version 
	if ! grep -q "AB-Solution 1.05" $jScripts/dnsmasq.postconf ;then
		# make a backup of it
		cp -f $jScripts/dnsmasq.postconf $installDirPath/$backupDir/dnsmasq.postconf_$todayHour
...
if [ $adBlockingState == "on" ];then
	...
	echo -e " sh $installDirPath/$scriptsDir/$updateHostsFile\n"
fi
...
write_config_file(){
	...
	echo "" >> $confFileLink
	if [ -f /jffs/absolution.backup ];then
		source /jffs/absolution.backup
